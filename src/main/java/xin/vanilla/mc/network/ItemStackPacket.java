...
public class ItemStackPacket {
     ...
     *
     * @param message 包含物品堆信息的数据包
     * @param ctx     用于访问网络事件上下文的供应商函数，包括访问玩家信息和设置数据包处理状态
     */
    public static void handle(ItemStackPacket message, Supplier<NetworkEvent.Context> ctx) {
        // 获取网络事件上下文并排队执行工作
        ctx.get().enqueueWork(() -> {
            // 获取发送数据包的玩家实体
            ...
            if (player != null) {
                // 尝试将物品堆添加到玩家的库存中
                boolean added = player.inventory.add(message.itemStack);

                // 如果物品堆无法添加到库存，则以物品实体的形式生成在世界上
                if (!added) {
                    ItemEntity itemEntity = new ItemEntity(player.level, player.getX(), player.getY(), player.getZ(), message.itemStack);
